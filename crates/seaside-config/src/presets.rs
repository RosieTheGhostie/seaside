...
pub trait HasBasicPresets: HasPresets<Presets = BasicPresets> {
    ...
    #[allow(
        dead_code,
        reason = "nothing uses this, but it makes sense to have around"
    )]
    fn recommended() -> Self {
...
macro_rules! impl_bitflags_has_basic_presets {
    ($flag:ty, $recommended:expr) => {
        impl $crate::presets::HasPresets for $flag {
            type Presets = $crate::presets::BasicPresets;
        ...
        impl $crate::presets::HasBasicPresets for $flag {}
    };
}
