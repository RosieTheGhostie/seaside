//! Explicit sign extension for primitive unsigned integer types.
//!
//! Provides the trait [`SignExtend`], which can sign-extend an unsigned integer to any signed
//! integer of a larger size.

/// Provides a method to sign-extend an unsigned integer.
pub trait SignExtend<T>
where
    Self: Sized,
    T: Sized,
{
    /// The signed counterpart of this integer.
    ///
    /// This should be the same size as the unsigned integer implementing this trait.
    type SignedSelf: Into<T> + Sized;

    /// Returns a sign-extended version of this integer.
