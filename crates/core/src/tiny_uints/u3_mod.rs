...
use super::{r#impl, impl_ops, u2, u5};
...
impl u3 {
    ...
    pub const fn as_u5(self) -> u5 {
        // SAFETY: All valid `u3`s are valid `u5`s.
    ...
    /// ```
    /// # use seaside_core::u3;
    /// # const ZERO: u3 = u3::ZERO;
    /// # const FIVE: u3 = u3::new(5).unwrap();
    /// assert_eq!(ZERO.most_significant_bit(), 0 as _);
    /// assert_eq!(FIVE.most_significant_bit(), 1 as _);
    /// assert_eq!(u3::MAX.most_significant_bit(), 1 as _);
    /// ```
    pub const fn most_significant_bit(self) -> bool {
        const SHIFT_AMOUNT: u32 = u3::BITS - 1;
    ...
    ///
    /// ```
    /// # use seaside_core::{u2, u3};
    /// # const FIVE: u3 = u3::new(5).unwrap();
    /// # const TWO: u2 = u2::new(2).unwrap();
    /// assert_eq!(FIVE.upper_two_bits(), TWO);
    ...
    /// assert_eq!(u3::MAX.least_significant_bit(), 1 as _);
    /// ```
    #[doc(alias = "is_odd")]
    ...
    /// assert_eq!(u3::MAX.lower_two_bits(), u2::MAX);
    /// ```
