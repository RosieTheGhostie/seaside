    ...
    fn write_u8(&mut self, address: Address, value: u8) -> Result<(), Exception> {
        if self.writeable {
            self.text
                ...
                .or_else(|_| self.ktext.write_u8(address, value))
        } else {
    ...
    fn write_u16(
        ...
        if self.writeable {
            self.text
                ...
                .or_else(|_| self.ktext.write_u16(address, value, assert_aligned))
        } else {
            Err(Exception::InvalidStore(address))
        }
    }
    ...
    fn write_u32(
        ...
        if self.writeable {
            self.text
                ...
                .or_else(|_| self.ktext.write_u32(address, value, assert_aligned))
        } else {
            Err(Exception::InvalidStore(address))
        }
    }
